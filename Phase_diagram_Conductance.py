...
E_F = np.linspace(-300, 300, 3000)  # Fermi energy
...
eVb = np.linspace(-0.8, 0.8, 200)                            # Finite voltage bias
G_bias = np.zeros((len(eVb), len(E_F)))                      # Thermal conductance vector
...
#%% Phase diagram for finite voltage bias

for i, bias in enumerate(eVb):

    sample_points_bias = int(bias / dE)  # Points included in the integration above and below
    E_F_bias = E_F[sample_points_bias: -sample_points_bias]  # Range over which we calculate the thermal conductance

    for j, energy in enumerate(E_F_bias):
        k = j + sample_points_bias  # Index in the complete E_f vector
        mu1, mu2 = E_F[k] + (0.5 * eVb), E_F[k] - (0.5 * eVb)  # Chemical potentials
        integration_interval = E_F[k - sample_points_bias: k + sample_points_bias]  # Energy range for integration
        G_interval = G[k - sample_points_bias: k + sample_points_bias]  # Conductance range for integration
        G_bias[i, j] = finite_voltage_bias(0, mu1, mu2, integration_interval, G_interval)  # Thermal averaged conductance
...
logG = np.log(G_bias)
im = plt.pcolormesh(eVb, E_F, G_bias)
cb = plt.colorbar(im)
